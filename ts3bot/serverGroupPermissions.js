...
serverGroupPermissions.resettingPermissions = function (config, serverQueryClient, callback) {
  if (config.adjustJoinPower === true) {
    for (var gameWorldIdAdjustJoinPower in config.gameWorlds) {
      ...
      serverQueryClient.send('servergroupaddperm', {
        sgid: config.gameWorlds[gameWorldIdAdjustJoinPower].serverGroupId,
        permsid: 'i_channel_join_power',
        permvalue: config.defaultJoinPower,
        permnegated: 0,
        permskip: 0
  ...
  if (config.adjustChannelSubscriptions === true) {
    for (var gameWorldIdAdjustChannelSubscriptions in config.gameWorlds) {
      ...
      serverQueryClient.send('servergroupaddperm', {
        sgid: config.gameWorlds[gameWorldIdAdjustChannelSubscriptions].serverGroupId,
        permsid: 'i_client_max_channel_subscriptions',
        permvalue: config.defaultChannelSubscriptions,
        permnegated: 0,
        permskip: 0
      }, function (error, response, rawResponse) {
        if (error) logger.log('error', 'Error while changing server group permission. ' + util.inspect(error))
  ...
  if (config.adjustSubscribePower === true) {
    for (var gameWorldIdAdjustSubscribePower in config.gameWorlds) {
      ...
      serverQueryClient.send('servergroupaddperm', {
        sgid: config.gameWorlds[gameWorldIdAdjustSubscribePower].serverGroupId,
        permsid: 'i_channel_subscribe_power',
        permvalue: config.defaultSubscribePower,
        permnegated: 0,
        permskip: 0
      }, function (error, response, rawResponse) {
        if (error) logger.log('error', 'Error while changing server group permission. ' + util.inspect(error))
...
serverGroupPermissions.elevatingPermissins = function (config, serverQueryClient, callback) {
  config.worldsAllowed.forEach(function (allowedGameWorld) {
    if (config.adjustJoinPower === true) {
      ...
      serverQueryClient.send('servergroupaddperm', {
        sgid: config.gameWorlds[allowedGameWorld].serverGroupId,
        permsid: 'i_channel_join_power',
        permvalue: config.elevatedJoinPower,
        permnegated: 0,
        permskip: 0
      }, function (error, response, rawResponse) {
        if (error) logger.log('error', 'Error while changing server group permission. ' + util.inspect(error))
    ...
    if (config.adjustChannelSubscriptions === true) {
      ...
      serverQueryClient.send('servergroupaddperm', {
        sgid: config.gameWorlds[allowedGameWorld].serverGroupId,
        permsid: 'i_client_max_channel_subscriptions',
        permvalue: config.elevatedChannelSubscriptions,
        permnegated: 0,
        permskip: 0
      }, function (error, response, rawResponse) {
        if (error) logger.log('error', 'Error while changing server group permission. ' + util.inspect(error))
    ...
    if (config.adjustSubscribePower === true) {
      ...
      serverQueryClient.send('servergroupaddperm', {
        sgid: config.gameWorlds[allowedGameWorld].serverGroupId,
        permsid: 'i_channel_subscribe_power',
        permvalue: config.elevatedSubscribePower,
        permnegated: 0,
        permskip: 0
      }, function (error, response, rawResponse) {
        if (error) logger.log('error', 'Error while changing server group permission. ' + util.inspect(error))
